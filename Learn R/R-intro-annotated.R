...
## Instead of copying 2 + 3 into the console, click on or highlight the line and run cmd-return or cntl-R 
...
#The results are different because of PEMDAS, the parenthesis go first.
...
## Type this into the console: functions replace algebraic operators
sum(2, 3)
...
#Now type:
Sum(2, 3) #r distinguishes lower case from upper case, runs exactly what you write
...
y <- x/8
y #prints our results of y

##QUESTION: What is the value of y?
#environment keeps your objects in the top right quadrant
#if x is 5 then 5/8 = 0.625, y  = 0.625. 
...
r <- rnorm(1000)
length(r)
#Z concatenate of one vector, z is a length of 3
...
## Type:
z * 2 +100 #APPLY TO EACH INSTALLEMNT IN VECTOR

## Create a new vector my_sqrt:
my_sqrt <- sqrt(z-1)

## QUESTION: Before you look at it, what do you think my_sqrt contains?
# The vector my_sqrt holds the square root results after subtracting 1 from each element.
...
20:1 #counts down from highest to least
-1:20 #count from -1 to 20 in order
-(1:20) # all numbers are negative, counts from -1 to -20
## The seq() function gives us more control over the sequence
## Type:
seq(1, 20) #power in generating a  seq
...
seq(0, 10, by=.5) #seq by 0.5 between 0-10, functions have arguments
#beginig 0, ending: 10, by 0.5 is specifying the arguement

## Or maybe we don't care about the increment, we just want 30 numbers between 5 and 10.
## Try it out:
my_seq <- seq(5, 10, length=30)
#function saved as my seq

## QUESTION: What is my_seq? a vector that lists values that starts at 5 and ends at 10 with 30 values. 
## TASK: Confirm that my_seq has length 30.

length(my_seq) #confirmed!
...
rep(c(0, 1, 2, 3, 4), times=10) #repeat the argument(0-4) in order 10 times

## EXTRA TASK: Can you generate the same vector by integrating the seq() and rep() functions?
rep(seq(0, 4, by = 1), times = 10) #seq 0-4 by increments of 1, the repeat it 10 times
#################################################
##6) Values can be characters as well as numbers.
#################################################

## What's your favorite species?
## Mine is Lasthenia californica (goldfields!)
## To make sure we remember that, let's make a vector called "drhalletts_favorite":
drhalletts_favorite <- c("Lasthenia", "californica")

## View that vector. But typing drhalletts_favorite takes a long time. 
## To save time, type dr then press tab. What happens? It creates drop down of options for different function names
dr

## TASK: Use the function length to confirm the length of drhalletts_favorite
length(drhalletts_favorite)
...
length(drhalletts_favorite2) #combined the 2 words in
## EXTRA TASK: Use the function 'paste' to join together the genus and species of your 
## favorite species. 

elisaherters_favorite<- c("Hydrochoerus", "hydrochaeris")
length(elisaherters_favorite)

elisaherters_favorite2<- paste("Hydrochoerus", "hydrochaeris") #paste the 2 words together
length(elisaherters_favorite2)

## EXTRA TASK: Concatenate c() your favorite species and mine (drwebsters_favorite2) 
## in a vector called our_favorites.
drwebsters_favorite <- c("Plantago","erecta") #the special guest's
drwebsters_favorite2 <- paste("Plantago","erecta")
our_favorites <- c(drwebsters_favorite2, elisaherters_favorite2)
our_favorites
length(our_favorites) #1 for Webster's and 1 for heater's favorite
################################################################################
...
dim(firstvector) #4 6
class(firstvector) #MATRIX ARRAY
...
students <- c("Lainie", "Josh", "Eli", "Daela")
...
#The output adds one column for student names, and converts all values to character while remaining a matrix.
...
#A data frame allows columns with different data types such as strings, integers, characters, booleans. 
...
happydata #prints out the matrix
## TASK: Find the class and dimension of happydata.
class(happydata) #its a dataframe
dim(happydata) # 4 7 
...
cnames <- c("students", "X1", "X2", "X3", "X4", "X5", "X6")
...
identical(firstmatrix, secondmatrix)  #Did we succeed? yes it outputted TRUE!
...
roll2 <- function(bones = 1:6){ #bones is the arguement
  dice <- sample(bones, size = 2, replace = TRUE) #random sampling using x
  sum(dice)
}

## roll2 is the name of the function. What are the other components of the function?
# some components include sample and sum
## What happens if you run roll2 without supplying any arguments?
roll2()
  #it'll use the default bones (1 through 6) from the original argument.Will output any random number between 1-6. 
...
## What happened? Why? 
#and error occured because the function pivot_longer is not found
...
## Now what happens when you print pivot_longer?
  #prints out <environment: namespace:tidyr>
...
#it outputted a table that's longer rather than wider. It created a more detailed data that shows each time point for each student. 
...
## argument 'sep'.Used as a space, joining 2 words using a separator. 

## TASK: Does this argument have a predetermined value? What is that value? yes the value is "_"
...
## you will be working through swirl modules for your first problem set.

...
# If swirl does not load below, please make sure it is installed
# install.packages("swirl")
library(swirl)

# I. R Programming -> 4. Vectors  
# Copy your assignment of num_vect below. Create a new vector that is num_vect divided by 3
num_vect <- 6:1 #declare variable, took from intro to R assignment but instead of 1-20 i did 1-6
new_vect <- num_vect / 3 #create new function to place equation in, divide each value by 3 , length should be 6
new_vect #prints it out in a vector
...
y <- rnorm(1000)       #given values
z <- rep(NA, 1000) #vector length of 1000 with NA values

my_data <- c(y, z) #combine both y and z in one variable
my_data2 <- c(y, z) #repeats it again and put it in a new variable data2

identical(my_data, my_data2) #checks if both data and data2 are same/identical
#The output read True that they re both identical because y and z are a length of 1000,and the order is the same. 
...
vect <- c(10, 15, 25) #created avector under the variable vect
vect2 <- vect  #put the vector into a new variable called vect2
names(vect2) <- c("first", "second", "third") #saved names to the three values in the vect2
vect2 #prints it out with the name changes
...
patientnumber <- c(1,2,3,4) #declare the variables
patients <- c("John", "Jane", "George", "Scott") 
patient_matrix <- cbind(patientnumber, patients) #combine and bind variables into matrix, referenced the intro to R assignment happy data example
patient_data <- as.data.frame(patient_matrix) #turn matrix into dataframe
patient_data #prints out the data frame into a matrix 
...
logicvec <- c(5 == 5, 6 != 7, 6 >= 7, 2 < 3 | 3 < 3) #created vector based off of provided arguements

logicvec #prints out true or false based on logic arguments

isTRUE(logicvec) #not all are true so isTRUE returns FALSE. 
...
# c <- a + b   #c= 1+2
# b <- 4      #b = 4
# a <- b   # b is is 4, takes B value
# c <- a   # c is 4 takes a value
# c 

# Answer:c is 4 because c was given the value of A and a was changed to 4 by B value. 
...
# log(x = 1000, base = 10) #log_10 1000
# log10(1000) # same thing as first but in shortcut
# log(base = 10, x = 1000) #the order changed in parenthesis but arguments are declared already so order doesnt matter 
# log(10, 1000)  #no arguments declared so it looks like log base 1000 of 10 which is diff

# Answer: log(10, 1000) because  10 is treated as x, 1000 as base. So it looks like this log base 1000 of 10
...
numvector <- c(5,2,3,1,6,8) #what was provided
newvector <- sort(numvector, decreasing = TRUE) #use argument sort to make it in descending order
newvector  #prints in the order of the argument
...
elephant1_kg <- 3492 #given 
elephant2_lb <- 7757 # already in ibs unit, no need to convert

elephant1_lb <- elephant1_kg * 2.2  #use conversion to turn elephant 1 from kg to ibs

elephant1_lb > elephant2_lb  #argument to find out who weights more using true or false
# It printed FALSE so elephants 2 weighs more.
...
# In what ways is this data set "big data"? In what ways is it not big data?
  
# Answer: The data set is big because theirs 20 towers and your measuring multiple variables such as CO2 and wind speed. Plus the data is continously being collected. 
#the data set could be not big because its not a global or country data set and theres only 20 towers which isnt super large number. 
